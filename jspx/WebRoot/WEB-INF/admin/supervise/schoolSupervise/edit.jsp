...
function approvalFile(id,status){
  ...
  <body >
     ...
     <div data-options="region:'center'"  >
     	<!-- <table id="mxList" class="easyui-datagrid"  >   
		    <thead>   
		        <tr>   
		        	<th data-options="field:'projectId'"   checkbox=true></th>   
		            <th data-options="field:'projectName'"   width="10">项目名称</th>   
		            <th data-options="field:'remark'"    width="30">项目说明</th>
		            <th data-options="field:'schoolMaterials'"    width="30">学校材料</th>
		            <th data-options="field:'checkMaterials'"    width="30">督学下校检查</th>
		            <th data-options="field:'modifyMaterials'"    width="30">整改处理</th>
		        </tr>   
		    </thead>   
		    <tbody>   
		    </tbody>   
		...
		<table width="100%" class="table table-hover table-condensed">
			...
			<c:forEach items="${projectList }" var="project" varStatus="i">
				<tr>
					...
					<c:if test="${i.index==0 }">
						<td rowspan="${projectList.size() }" >${project.checkMaterials }</td>
					</c:if>
					<c:if test="${i.index==0 }">
						<td rowspan="${projectList.size() }" >${project.modifyMaterials }</td>
					</c:if>
				</tr>
			</c:forEach>
		</table>
