 ...
 * 打开添加数据窗口参数
 */
var options = {id:"d3",title:"站内短信",width:"100%",height:"100%",toolbar:dbuttons};
//添加函数
function  addF(){
	var urls = "../train/noticeManagement/addSMS.do";
	options.urls = urls;
 ...
 */
function  loadF(row){
	var urls = "../train/noticeManagement/loadSMS.do?id="+row.id;
	options.urls = urls;
	parent.$.createDialog(options);
	parent.$.createDialog.open_grid = dataGrid;
}
 ...
 */
function  look(row){
	var parameter = {id:"d3",title:"站内短信",width:"100%",height:"100%"};
	var urls = "../train/noticeManagement/lookTeacher.do?id="+row.id;
	parameter.urls = urls;
